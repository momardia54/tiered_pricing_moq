...
def validate_moq(doc, method):
    ...
    item_details = frappe.db.get_value("Item", item.item_code, ["custom_minimum_order_quantity", "stock_uom"], as_dict=True)
    
    if item_details:
        min_qty = item_details.custom_minimum_order_quantity
